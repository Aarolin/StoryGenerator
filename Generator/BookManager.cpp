...
std::vector<Relationship> BookManager::readFile(const std::string& filename, WordDictionary& dict)
{
	std::filesystem::path p(filename);
	return readFile(p, dict);
}

std::vector<Relationship> BookManager::readFile(const std::filesystem::path p, WordDictionary& dict)
{
	parser_.openFile(p);
...
void BookManager::writeFile(const std::string& filename, const std::string& data) const
{
